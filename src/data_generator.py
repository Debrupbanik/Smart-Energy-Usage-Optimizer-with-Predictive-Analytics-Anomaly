...
def generate_synthetic_data(days=30):
    ...
    # Noise
    noise = np.random.default_rng().normal(0, 2, hours)
    ...
    anomalies_indices = rng.choice(hours, size=int(hours * 0.01), replace=False)
    consumption[anomalies_indices] += rng.choice([-20, 20], size=len(anomalies_indices))
